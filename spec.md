...
The prompt provided to each LLM agent will encapsulate all pertinent information for its decision-making turn. The prompt is structured as a conversation with a `system` message containing the game state and a `user` message containing the discussion transcript. Example:  

**System Message:**
```
<system-instruction>You are playing One Night Ultimate Werewolf. Your objective is to secure victory for your team. Formulate strategic statements and votes. Respond with a JSON object containing publicStatement, privateRoleGuesses, and vote fields.</system-instruction>
<game-state>
<players>PlayerA, PlayerB, PlayerC, PlayerD</players>
<possible-roles>Villager, Werewolf, Seer, Troublemaker, Robber</possible-roles>
<center-cards>[Card1, Card2, Card3] (hidden from players)</center-cards>
</game-state>
<your-player-id>PlayerB</your-player-id>
<your-initial-role>Seer.</your-initial-role>
<your-night-observations>You observed PlayerB as a Werewolf.</your-night-observations>
```
...
5.2. Data Format:  
Each entry in the SFT dataset will be a JSON object representing a conversation, with a list of messages. Each message has a `role` (`system`, `user`, or `assistant`) and `content`.

*   **System Message:** The content will be the system prompt containing all game state information, as detailed in Section 4.1.
*   **User Message:** The content will be the discussion transcript.
*   **Assistant Message:** The content will be the desired structured JSON output with camelCase field names, including `publicStatement`, `privateRoleGuesses`, and `vote`.
